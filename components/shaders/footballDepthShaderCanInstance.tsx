...
const vertexShader = (pointCount: number) => /* glsl */ `
  ...
  varying mat3 vViewNormalMatrix;
  ...
  void main() {
    ...
    vBallNormal = normal;
      ...
      mat4 currentModelMatrix = modelMatrix * instanceMatrix;
    #else
      mat4 currentModelMatrix = modelMatrix;
    #endif

    vec4 worldPos = currentModelMatrix * vec4(displacedPosition, 1.0);
    vWorldPosition = worldPos.xyz;
    vWorldNormal = normalize(mat3(currentModelMatrix) * normal);

    #ifdef USE_INSTANCING
      vViewNormalMatrix = normalMatrix * mat3(instanceMatrix);
    #else
      vViewNormalMatrix = normalMatrix;
    #endif
  }
`;
...
const fragmentShader = (pointCount: number) => /* glsl */ `
  ...
  vec3 getLocalPerturbedNormal(vec3 spherePosition, vec3 baseNormal, float nearestDist, float secondDist) {
    ...
    float nearestDistLeft, secondDistLeft, typeLeft;
    float nearestDistRight, secondDistRight, typeRight;
    float nearestDistDown, secondDistDown, typeDown;
    float nearestDistUp, secondDistUp, typeUp;
    ...
    vec3 perturbedLocalNormal = normal
      - edgeHeight * normal
      - tangentSlope * uBumpStrength * 0.5 * tangent
      - bitangentSlope * uBumpStrength * 0.5 * bitangent;

    return normalize(perturbedLocalNormal);
  }

  float sphereRim() {
    vec3 viewDir = normalize(cameraPosition - vWorldPosition);
    float rim = 1.0 - max(dot(viewDir, normalize(vWorldNormal)), 0.0);
  ...
  void main() {
    ...
    vec3 perturbedLocalNormal = getLocalPerturbedNormal(vBallPosition, vBallNormal, nearestDist, secondDist);

    csm_FragNormal = normalize(vViewNormalMatrix * perturbedLocalNormal);
...
export default function FootballDepthShaderCanInstance() {
